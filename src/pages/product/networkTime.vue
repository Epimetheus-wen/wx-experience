<template>
	<view class="networkTime">
		<Header :titleTop="titleTop"></Header>
		<view class="networkTime_main">
			<view class="banner">
				<image class="img" src="../../static/privacy/banner@1x.jpg" mode=""></image>
			</view>
			<view class="networkTime_center">
				<view class="networkTime_header">
					<view class="title">
						在网时长查询
					</view>
				...
				<view class="networkTime_from">
					<view class="networkTime_input">
						<view class="left">
                    ...
                    <view class="networkTime_input networkTime_puzz">
						<view class="left">
				...
				<view class="networkTime_footer">
					...
					<view class="num">剩余可试用：<text>{{count}}</text>次</view>
                    <suspendButton :haibaoObj='haibaoObj' @event='event' :refreshUrl="'networkTime'"></suspendButton>
				</view>
	...
	export default {
            ...
            uni.getStorage({
                ...
                success: function(res) {
                    ...
                    if (res.data && res.data != undefined && res.data != "undefined") {
                        that.Phone = res.data;
                        that.$getRequestCount(64,res.data)
                            .then((response) => {
		...
		methods: {
			...
			generate(){
                ...
                that.loading=true
				wx.request({
					url: that.$reUrl+"/console/trialRequest.html?f=networkNumberVerify",
					data: {
                        phone: that.mobile,
					...
					success(ress) {
                        ...
                        if(ress.data.respCode=='000'){
                            ...
                            let rangeStart=ress.data.data.rangeStart
                            let rangeEnd=ress.data.data.rangeEnd
                            if(rangeStart==0&&rangeEnd==0){
                                that.newText="最近入网"
                            }
                            if(rangeStart==0&&rangeEnd==3){
                                that.newText="3个月内"
                            }
                            if(rangeStart==3&&rangeEnd==6){
                                that.newText="半年内"
                            }
                            if(rangeStart==6&&rangeEnd==12){
                                that.newText="1年内"
                            }
                            if(rangeStart==12&&rangeEnd==24){
                                that.newText="2年内"
                            }
                            if(rangeStart==24&&rangeEnd==-1){
                                that.newText="2年以上"
                            }
                            that.$getRequestCount(64,that.Phone)
                                .then((response) => {
                                    that.count=response.data.count
