<template>
	<view class="bankCardFour">
		<Header :titleTop="titleTop"></Header>
		<view class="bankCardFour_main">
			<view class="banner">
				<image class="img" src="../../static/privacy/banner@1x.jpg" mode=""></image>
			</view>
			<view class="bankCardFour_center">
				<view class="bankCardFour_header">
					<view class="title">
						银行卡四要素核验
					</view>
				...
				<view class="bankCardFour_from">
                    <view class="bankCardFour_input">
						<view class="left">
                    ...
                    <view class="bankCardFour_input bankCardFour_puzz">
						<view class="left">
				...
				<view class="bankCardFour_footer">
					<view class="hint">
						<view class="title">
					...
					<view class="num" style="padding-bottom: 50rpx;">剩余可试用：<text>{{count}}</text>次</view>
                    <suspendButton :haibaoObj='haibaoObj' @event='event' :refreshUrl="'bankCardFour'"></suspendButton>
				</view>
	...
	export default {
		...
		components: {
         ...
         onShareAppMessage(res) {
			...
			return {
				...
				path: '/pages/product/bankCardFour',
            ...
            uni.getStorage({
                ...
                success: function(res) {
                    ...
                    if (res.data && res.data != undefined && res.data != "undefined") {
                        that.Phone = res.data;
                        that.$getRequestCount(74,res.data)
                            .then((response) => {
		...
		methods: {
			...
			generate(){
                ...
                this.loading=true
				wx.request({
					url: that.$reUrl+"/console/trialRequest.html?f=identityVerifyFourBankCard",
					data: {
                        idcard: that.idCode,
                        name: that.name,
					...
					success(ress) {
                        ...
                        if(ress.data.respCode=='000'){
                            ...
                            that.$getRequestCount(74,that.Phone)
                                .then((response) => {
                                    that.count=response.data.count
