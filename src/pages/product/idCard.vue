<template>
	<view class="idCard">
		...
		<view class="idCard_main">
			<view class="banner">
				<image class="img" src="../../static/privacy/banner@1x.jpg" mode=""></image>
			</view>
			<view class="idCard_center">
				<view class="idCard_header">
					<view class="title">
						身份证实名认证
					</view>
				...
				<view class="idCard_from">
                    ...
                    <view class="idCard_input idCard_puzz">
						<view class="left">
				...
				<view class="idCard_footer">
					...
					<view class="num">剩余可试用：<text>{{count}}</text>次</view>
                    <suspendButton :haibaoObj='haibaoObj' @event='event' :refreshUrl="'idCard'"></suspendButton>
				</view>
	...
	export default {
            ...
            uni.getStorage({
                ...
                success: function(res) {
                    ...
                    if (res.data && res.data != undefined && res.data != "undefined") {
                        that.Phone = res.data;
                        that.$getRequestCount(71,res.data)
                            .then((response) => {
		...
		methods: {
			...
			generate(){
                ...
                that.loading=true
				wx.request({
					url: that.$reUrl+"/console/trialRequest.html?f=identityVerifyAutonym",
					data: {
                        idcode: that.idCode,
                        name: that.name,
					...
					success(ress) {
                        ...
                        if(ress.data.respCode=='000'){
                            ...
                            that.$getRequestCount(71,that.Phone)
                                .then((response) => {
                                    that.count=response.data.count
