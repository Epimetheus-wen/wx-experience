<template>
	<view class="realTime">
		...
		<view class="realTime_main">
			<view class="banner">
				<image class="img" src="../../static/privacy/banner@1x.jpg" mode=""></image>
			</view>
			<view class="realTime_center">
				<view class="realTime_header">
					<view class="title">
						号码实时状态检测
					</view>
				...
				<view class="realTime_from">
					<view class="realTime_input">
						<view class="left">
                    ...
                    <view class="realTime_input realTime_puzz">
						<view class="left">
				...
				<view class="realTime_footer">
					...
					<uni-popup class="pop" ref="popup1" type="center" @change='handelChange'>
						<view class="center">
							...
							<view class="txt">
                                检测结果：<text class="type_item" :style="newText=='正常'?'color:#00b249':newText=='空号'?'color:#000000':newText=='关机'?'color:#ff3434':newText=='忙'?'color:#ff981f':newText=='停机'?'color:#fb0000':'color:#000000'">{{newText}}</text>（
                                <text class="type_item1" :style="type[0]=='正常'?'color:#00b249':type[0]=='空号'?'color:#000000':type[0]=='关机'?'color:#ff3434':type[0]=='忙'?'color:#ff981f':type[0]=='停机'?'color:#fb0000':'color:#000000'">{{type[0]}}</text>、
                                <text class="type_item2" :style="type[1]=='正常'?'color:#00b249':type[1]=='空号'?'color:#000000':type[1]=='关机'?'color:#ff3434':type[1]=='忙'?'color:#ff981f':type[1]=='停机'?'color:#fb0000':'color:#000000'">{{type[1]}}</text>、
                                <text :style="type[2]=='正常'?'color:#00b249':type[2]=='空号'?'color:#000000':type[2]=='关机'?'color:#ff3434':type[2]=='忙'?'color:#ff981f':type[2]=='停机'?'color:#fb0000':'color:#000000'" class="type_item3">{{type[2]}}</text>、
                                <text :style="type[3]=='正常'?'color:#00b249':type[3]=='空号'?'color:#000000':type[3]=='关机'?'color:#ff3434':type[3]=='忙'?'color:#ff981f':type[3]=='停机'?'color:#fb0000':'color:#000000'" class="type_item3">{{type[3]}}</text>）
							</view>
					...
					<view class="num">剩余可试用：<text>{{count}}</text>次</view>
                    <suspendButton :haibaoObj='haibaoObj' @event='event' :refreshUrl="'realTime'"></suspendButton>
				</view>
	...
	export default {
            ...
            uni.getStorage({
                ...
                success: function(res) {
                    ...
                    if (res.data && res.data != undefined && res.data != "undefined") {
                        that.Phone = res.data;
                        that.$getRequestCount(63,res.data)
                            .then((response) => {
		...
		methods: {
			...
			generate(){
                ...
                that.loading=true
				wx.request({
					url: that.$reUrl+"/console/trialRequest.html?f=rtNumberVerify",
					data: {
                        phone: that.mobile,
					...
					success(ress) {
                        ...
                        if(ress.data.respCode=='000'){
                            ...
                            that.$getRequestCount(63,that.Phone)
                                .then((response) => {
                                    that.count=response.data.count
