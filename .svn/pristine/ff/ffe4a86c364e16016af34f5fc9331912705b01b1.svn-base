<template>
	<view class="vacantNumber">
		<Header :titleTop="titleTop"></Header>
		<view class="vacantNumber_main">
			<view class="banner">
				<image class="img" src="../../static/privacy/banner@1x.jpg" mode=""></image>
			</view>
			<view class="vacantNumber_center">
				<view class="vacantNumber_header">
					<view class="title">
						空号检测
					</view>
				...
				<view class="vacantNumber_from">
					<view class="vacantNumber_input">
						<view class="left">
                    ...
                    <view class="vacantNumber_input vacantNumber_puzz">
						<view class="left">
				...
				<view class="vacantNumber_footer">
					<view class="hint">
						<view class="title">
					...
					<view class="num">剩余可试用：<text>{{count}}</text>次</view>
                    <suspendButton :haibaoObj='haibaoObj' @event='event' :refreshUrl="'vacantNumber'"></suspendButton>
				</view>
	...
	export default {
            ...
            uni.getStorage({
                ...
                success: function(res) {
                    ...
                    if (res.data && res.data != undefined && res.data != "undefined") {
                        that.Phone = res.data;
                        that.$getRequestCount(61,res.data)
                            .then((response) => {
		...
		methods: {
			...
			generate(){
                ...
                that.loading=true
				wx.request({
					url: that.$reUrl+"/console/trialRequest.html?f=vacancyNumberVerify",
					data: {
                        phone: that.mobile,
					...
					success(ress) {
                        ...
                        if(ress.data.respCode=='000'){
                            ...
                            that.$getRequestCount(61,that.Phone)
                                .then((response) => {
                                    that.count=response.data.count
