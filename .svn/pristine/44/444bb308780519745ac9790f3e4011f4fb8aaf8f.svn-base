<template>
	<view class="voice">
		...
		<view class="voice_main">
			<view class="banner">
				<image class="img" src="../../static/privacy/banner@1x.jpg" mode=""></image>
			</view>
			<view class="voice_center">
				<view class="voice_header">
					<view class="title">
						语音通知
					</view>
					<view class="xq_btn" @tap="wailian('product/voice')">
						<text>产品详情</text>
						<image class="img" src="../../static/privacy/sanjiaoxing@1x.png" mode=""></image>
					</view>
				</view>
				<view class="voice_from">
                    ...
                    <view class="voice_input voice_puzz">
						<view class="left">
				...
				<view class="voice_footer">
					...
					<button class="generate" type="primary" :loading='loading' :disabled='loading' @tap="generate"  v-text="loading?' 正在发送中':'接收语音'">接收语音</button>
                    <uni-popup class="pop" ref="popup1" type="center" @change='handelChange'>
						<view class="center">
							...
							<view class="title">
								手机号<text>{{newMobile}}</text>将听到以下内容：
							</view>
							<view class="txt">
                                {{newText}}
							</view>
					...
					<view class="num">剩余可试用：<text>{{count}}</text>次</view>
                    <suspendButton :haibaoObj='haibaoObj' @event='event' :refreshUrl="'voice'"></suspendButton>
				</view>
...
<script>
    ...
    export default {
        ...
        onReady() {
            let that = this;
            that.$authorization()
            console.log("that.$reUrl",that.$reUrl)
            uni.getStorage({
                key: "phone",
                success: function(res) {
                    ...
                    if (res.data && res.data != undefined && res.data != "undefined") {
                        that.Phone = res.data;
                        that.$getRequestCount(0,res.data)
                            .then((response) => {
		...
		methods: {
			...
			generate(){
                ...
                that.loading=true
				wx.request({
					url: that.$reUrl+"/console/trialRequest.html?f=voiceInform",
					data: {
                        mobile: that.mobile,
					...
					success(ress) {
                        ...
                        if(ress.data.code==0){
                            ...
                            that.$getRequestCount(0,that.Phone)
                                .then((response) => {
                                    that.count=response.data.count
